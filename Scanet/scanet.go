...
func Help() {
	fmt.Println("\tMENU DE AJUDA:")
	fmt.Println("\t-h or --help -> Abre a opção de ajuda")
	fmt.Println("\t-V -> Verifica as melhores opções DNS")
	fmt.Println("\t-T -> Troca para a melhor opção DNS")
}

func main() {
	help := flag.Bool("h", false, "Opção de ajuda")
	help1 := flag.Bool("help", false, "Opção de ajuda")
